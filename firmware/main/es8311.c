...
static inline esp_err_t es8311_write_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t data)
{
    ...
    return i2c_master_transmit(es->dev, buf, sizeof(buf), ES8311_I2C_TIMEOUT_MS);
}

static inline esp_err_t es8311_read_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t *reg_value)
{
    es8311_dev_t *es = (es8311_dev_t *) dev;
    return i2c_master_transmit_receive(es->dev, &reg_addr, 1, reg_value, 1, ES8311_I2C_TIMEOUT_MS);
}
...
void es8311_delete(es8311_handle_t dev)
{
    if (!dev) return;
    es8311_dev_t *es = (es8311_dev_t *) dev;
    if (es->dev) i2c_master_bus_rm_device(es->dev);
    free(dev);
}
...
es8311_handle_t es8311_create(i2c_master_bus_handle_t bus, const uint16_t dev_addr)
{
    ...
    i2c_device_config_t dev_cfg = {
        ...
        .device_address = (uint8_t)dev_addr,
        .scl_speed_hz = 100000,
    };
