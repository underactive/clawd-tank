...
- **Audio output on fnk0104** — The Freenove ESP32-S3 2.8" board now plays short sound effects alongside Clawd's working animations, using its on-board ES8311 codec, speaker-amp, and speaker. Five clips are mapped to animation states: keyboard-typing clicks under `CLAWD_ANIM_TYPING` (Edit-family tools), muffled hammer taps under `CLAWD_ANIM_BUILDING` (Bash), a brief sci-fi hum under `CLAWD_ANIM_THINKING`, a searching/scanning blip under `CLAWD_ANIM_DEBUGGER` (Read-family), and a single soft notification tick on every `notification:add`. Animation sounds are edge-triggered — they play once on the transition into the animation, not on every frame. A new `BOARD_HAS_AUDIO` compile flag gates the whole subsystem; the Waveshare C6 board and simulator build with audio disabled. Volume is fixed at `BOARD_AUDIO_VOLUME_PCT = 60` in `board_config.h` — the ES8311 register mapping is linear in dB, so perceptually most of the range lives between 85–100 %; 60 works well against a quiet room but will feel soft in noisy environments.
- **Shared I2C master bus** — A new `i2c_bus.c/h` module owns the fnk0104's I2C0 peripheral and lazily creates the master bus on first call to `i2c_bus_get()`. Touch and the audio codec now share the bus instead of each creating their own. The previous `BOARD_TOUCH_I2C_SDA/SCL` constants are renamed to `BOARD_I2C_SDA/SCL` since they are no longer touch-specific.
- **Vendored ES8311 codec driver** — `firmware/main/es8311.{c,h}` + `es8311_reg.h`, ported from the `pixel-agents-esp32` project. The port replaces the legacy `driver/i2c.h` calls with the new `driver/i2c_master.h` API (`i2c_master_bus_add_device`, `i2c_master_transmit`, `i2c_master_transmit_receive`) so it coexists with the shared bus. The Espressif managed component was rejected because it still uses the legacy I2C driver and cannot coexist with a new-driver bus handle.
- **"Flip Display 180°" menubar toggle** — A new menu item below Session Timeout lets users who mount the device upside-down rotate the image without reflashing. Under the hood: the menubar sends a `{"display_flipped": true|false}` config write over BLE; the firmware persists it to NVS under `disp_flip` (loaded at boot) and applies the rotation by XOR-ing `esp_lcd_panel_mirror`'s flags against the board's native `BOARD_LCD_MIRROR_X/Y`, then forcing a full-screen LVGL invalidate + synchronous refresh so every widget (including static sky/ground layers) re-renders through the post-flip CASET/RASET mapping. Without the invalidate, only dynamic widgets like the Clawd sprite and the time label would appear flipped while static scene layers stayed in their pre-flip panel-RAM positions. The mirror write and invalidate run on the UI task via a new `BLE_EVT_SET_DISPLAY_FLIPPED` queue event so the SPI bus and LVGL aren't touched from the NimBLE host task. Device is the source of truth for the flipped state — unplugging and reconnecting to a different Mac preserves the orientation. The protocol change is additive and non-breaking; no GATT version bump. Gated on real hardware only (simulator build compiles the path but has nothing to rotate); the C6 board's 172-pixel ST7789 with `y_gap=34` hasn't been tested and may need a corresponding gap adjustment after the mirror flip.
...
- **Sky gradient banding on the fnk0104's 240-row screen** — The night-sky gradient used endpoints `0x0a0e1a → 0x1a1a2e`, a spread of only ~12–20 steps per RGB channel. Over 240 rows that meant each color step had to stretch across 12–20 rows, producing visible horizontal seams the eye read as "tiling" of a shorter background. LVGL 9.5's `LV_GRAD_DIR_VER` renderer already sizes its color LUT to the widget height (`lv_draw_sw_grad.c:102-103`), so the widget *was* spanning the full screen — the banding was latent in the gradient spec and hid on the 172-row C6 panel because bands landed in ~8-row chunks. Endpoints widened to `0x02030a → 0x26264a` (~60+ steps per channel) so every row maps to a distinct color. Also added a `display_set_flipped` stub in the simulator's `sim_display_shim.c` so the shared firmware sources continue to link against the simulator target.
- **Stars only spanned the left half of the sky** — The six hand-placed stars in `star_cfg` had x-coordinates in the 10..160 range, leaving the right half of the 320-pixel-wide scene empty. A relic of an earlier layout era when the scene panel animated between narrow and wide widths; the scene has sat at full width for a while now. Pushed the right-most three stars out to 180 / 235 / 290 so the constellation spans the full width without losing its irregular, hand-placed feel. Sizes and colors unchanged.
